...
public class FlipView extends FrameLayout {
	...
	public interface OnFlipViewSwipeListener {
	...
	private static final int PEAK_ANIM_DURATION = 700; // in ms
	private static final int MAX_SINGLE_PAGE_FLIP_ANIM_DURATION = 440; // in ms
	...
	private static final int VERTICAL_FLIP = 0; // constant used by the attributes
	@SuppressWarnings("unused")
	private static final int HORIZONTAL_FLIP = 1; // constant used by the attributes
	...
	// keep track of swipe pointer
	private float mSwipeLastX = -1;
	private float mSwipeLastY = -1;
	...
	// swipe stuff
	private double SWIPE_MAX_OFF_PATH = 250;
    private double SWIPE_MIN_DISTANCE = 120;
    private double SWIPE_MAX_DISTANCE = 250;

	// how many views should keep in active.
	// by default is 3.
	private static final int mActiveViewCount = 5;
	...
	private OnFlipViewSwipeListener mOnSwipeListener;
	...
	// after modify, there are two items in one page.
	private int[] lstCurrentPageIds;
	...
	private void init() {
		...
		mTouchSlop = vConfig.getScaledPagingTouchSlop();
		mMinimumVelocity = vConfig.getScaledMinimumFlingVelocity();
		mMaximumVelocity = vConfig.getScaledMaximumFlingVelocity();
		
		if(isTablet) {
			// Tablet.
			SWIPE_MAX_OFF_PATH = (vConfig.getScaledTouchSlop() * 1.0);
			SWIPE_MIN_DISTANCE = (vConfig.getScaledPagingTouchSlop() * 2.5);
			SWIPE_MAX_DISTANCE = (vConfig.getScaledPagingTouchSlop() * 26.5);
		} else {
			// Normal device.
			SWIPE_MAX_OFF_PATH = (vConfig.getScaledTouchSlop() * 1.0);
			SWIPE_MIN_DISTANCE = (vConfig.getScaledPagingTouchSlop() * 1.5);
			SWIPE_MAX_DISTANCE = (vConfig.getScaledPagingTouchSlop() * 12.5);
		}
	...
	private void dataSetChanged() {
		...
		// Display two data per page.
		mPageCount = (int) Math.ceil(mAdapter.getCount()/2.0);
	...
	private void setCurrentPageIds(int currentPage){
		// lstCurrentPageIds
		//lstCurrentPageIds = new int[]{(((currentPage + 1) * 2) - 1), ((currentPage + 1) * 2)};
		lstCurrentPageIds = new int[]{(currentPage * 2), ((currentPage * 2) + 1)};
	}
	
	private int getNewPositionOfCurrentPage() {
		
		setCurrentPageIds(mCurrentPage);
		
		for(int id : lstCurrentPageIds) {
			if (mCurrentPageId == mAdapter.getItemId(id)) {
				return mCurrentPage;
			}
		}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (action == MotionEvent.ACTION_CANCEL
			...
			{
				mIsSwiping = false;
				if(mSwipeDistance >= SWIPE_MIN_DISTANCE && mSwipeDistance <= SWIPE_MAX_DISTANCE) {
                	if(mOnSwipeListener != null) {
                		mOnSwipeListener.onFlipViewSwipe(this, SwipeMode.onSwipeLeft);
                	}
				}
				mSwipeDistance = 0;
			}
		...
		case MotionEvent.ACTION_MOVE:
			...
			final float sdx = x - mSwipeLastX;
			final float sxDiff = Math.abs(sdx);
			final float sdy = y - mSwipeLastY;
			final float syDiff = Math.abs(sdy);
			
			if ((sxDiff > mTouchSlop && sxDiff > syDiff)) {
				mIsSwiping = true;
				mSwipeLastX = x;
				mSwipeLastY = y;
			}
		...
		case MotionEvent.ACTION_DOWN:
			...
			mSwipeLastX = MotionEventCompat.getX(ev, mActivePointerId);
			mSwipeLastY = MotionEventCompat.getY(ev, mActivePointerId);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		if ((!mIsFlipping || !mIsSwiping) && !mLastTouchAllowed) {
			return false;
		}
		...
		{   // swiping
			if(!mIsSwiping) {
				...
				final float x = MotionEventCompat.getX(ev, pointerIndex);
				final float xDiff = Math.abs(x - mSwipeLastX);
				final float y = MotionEventCompat.getY(ev, pointerIndex);
				final float yDiff = Math.abs(y - mSwipeLastY);
			...
			out_if: if(mIsSwiping) {
				final int swipePointerIndex = MotionEventCompat.findPointerIndex(ev, mActivePointerId);
				if (swipePointerIndex == -1) {
					mActivePointerId = INVALID_POINTER;
					break;
				}
				
				final float x = MotionEventCompat.getX(ev, swipePointerIndex);
				final float xDiff = Math.abs(x - mSwipeLastX);
				final float y = MotionEventCompat.getY(ev, swipePointerIndex);
				//final float yDiff = Math.abs(y - mSwipeLastY);
				
				//final float diffY = mSwipeLastY - y;
            	final float diffX = mSwipeLastX - x;
            	mSwipeLastX = x;
				mSwipeLastY = y;
				
            	if (xDiff < SWIPE_MAX_OFF_PATH) {
            		//Log.i(TAG, "break if statement.");
            		break out_if;
            	}
            	
                mSwipeDistance += diffX;
			}
		}
		{   // Flipping
			if (!mIsFlipping) {
	...
	private void addToActiveView(View v, int page, int viewType) {
		...
		if (mActivePageQueue.size() > mActiveViewCount) {
	...
	private void peak(boolean isPeak, boolean next, boolean once) {
		if(isPeak) {
	...
	public void setAdapter(ListAdapter adapter) {
		...
		mAdapter = adapter;
		// Display two data per page.
		mPageCount = adapter == null ? 0 : (int) Math.ceil(mAdapter.getCount()/2.0);
	 ...
	 *            view
	 */
	public void peakNext(boolean isPeak, boolean once) {
		if (mCurrentPage < mPageCount - 1) {
			peak(isPeak, true, once);
		}
	}
	 ...
	 *            view
	 */
	public void peakPrevious(boolean isPeak, boolean once) {
		if (mCurrentPage > 0) {
			peak(isPeak, false, once);
		}
	}
	...
	/**
	 * The OnSwipeListener will notify of swiping. This is a great
	 * listener to have when implementing swipe motion detection
	 * 
	 * @param onFlipViewSwipeListener
	 */
	public void setOnFlipViewSwipeListener(OnFlipViewSwipeListener onFlipViewSwipeListener) {
		this.mOnSwipeListener = onFlipViewSwipeListener;
	}
